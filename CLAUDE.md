...
位于上下篇导航下方、报纸容器组件之外，独立渲染。

评论区顶部隐藏标题栏，将 reaction 计数替换为一行提示文字：”欢迎一起交流~”，字号略大且加粗，起到欢迎语作用。

输入框边框为浅灰色，去掉 focus 高亮光晕，保持简洁。提交按钮为纯黑色，无阴影无边框。各交互元素统一使用圆角（8px）。

评论列表字号 15–16px，行高 1.8，时间线细线（1px）。整体配色为浅色方案，背景白色，边框和分隔线用浅灰（#eee），主色调黑色（#222），与博客整体风格一致。字体与正文保持统一。
