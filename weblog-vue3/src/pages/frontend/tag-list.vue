    ...
    <div class="np-archive-page" :class="{ 'dark': isDark }">
        <div class="np-archive-container">
            ...
            <div v-for="tag in tags" :key="tag.id"
                 :id="'tag-' + tag.id" class="np-cat-row">

                <div class="np-cat-name-wrap">
                    <h2 class="np-cat-name" @click="scrollToTag(tag.id)">{{ tag.name }}</h2>
                    <span class="np-cat-badge">{{ tag.articlesTotal }}</span>
                </div>

                <div class="np-cat-cards">
                    <div v-for="(article, idx) in getArticles(tag.id)" :key="idx"
                         class="np-index-card"
...
function loadTags() {
    getTagList({}).then((res) => {
        ...
        tags.value.forEach((tag) => {
            if (!tag.articlesTotal || tag.articlesTotal <= 0) {
                articlesMap[tag.id] = []
                return
            }
            getTagArticlePageList({
                current: 1,
                size: tag.articlesTotal,
                name: tag.name,
            }).then((r) => {
                if (r.success) articlesMap[tag.id] = r.data || []
            })
        })
    })
}
...
function scrollToTag(id) {
    document.getElementById('tag-' + id)?.scrollIntoView({ behavior: 'instant' })
}
